...
2026-08-20: LoRA 관련 코드(question_generator.py, router.py)를 전부 주석 처리하면서 이
파일의 테스트 전체도 같이 주석 처리했다 - 테스트 대상 엔드포인트 자체가 더는 존재하지
않는다. Tailscale 라우팅을 실제로 완성해서 LoRA 코드를 되살리면 이 파일도 같이 되살리면
된다.
"""
