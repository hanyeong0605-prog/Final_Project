...
def _gemini_polish(question: str) -> str | None:
    ...
    try:
        ...
        prompt = (
            ...
            "5) 문법은 멀쩡한데 질문 '설계' 자체가 이상한 아래 케이스는 다음 기준으로 처리해라:\n"
            "   a) 논리적으로 안 맞는 인과관계(예: 보안/안정성이 목적인 기술에 '성능 향상'을 "
            "물어보는 것)면, 그 어색한 인과관계 부분만 빼고 나머지 내용으로 자연스럽게 고쳐라\n"
            "   b) 정답이 없는 주관적 우열 비교를 요구하면(예: 'A와 B 중 어느 쪽이 더 뛰어난지'), "
            "우열을 묻는 대신 각각의 장단점이나 선택 기준을 묻는 객관적 질문으로 바꿔라\n"
            "   c) 실무 기술 역량 검증과 무관한 일반 소비자용 제품/서비스 사용법을 묻는 질문이면"
            "(예: 'Google Docs 사용 시 유의점') - 이건 고쳐도 여전히 면접 질문으로 부적절하니 "
            "DISCARD해라\n"
            "   d) 질문에 구체적인 기술/제품/서비스/API/라이브러리 이름이 등장하면, 그게 실제로 "
            "존재하는 것인지 스스로 확신이 서는 경우에만 그대로 유지해라. 처음 들어보거나, 두 "
            "단어를 억지로 합쳐놓은 것처럼 보이거나(예: '리액티브 애널라이저', 'Synaptics "
            "Path'처럼 그럴듯하지만 실존을 확신할 수 없는 이름), 확신이 서지 않으면 그 "
            "이름/용어를 언급하지 않는 일반적인 질문으로 바꾸거나(예: '~와 관련해서 성능을 "
            "개선했던 경험을 말씀해 주세요'), 그렇게도 못 고치겠으면 DISCARD해라 - 모르는 "
            "용어는 추측으로 진짜인 척 넘어가지 말고 반드시 의심해라\n"
            "   e) 서로 명확히 무관한 두 가지 주제/기술을 한 질문에 억지로 같이 묶어놓은 "
            "경우(예: '웹 접근성과 로우코드 도입을 비교하면')면, 원래 후보에서 더 핵심적으로 "
            "보이는 주제 하나만 남기고 나머지 주제는 질문에서 완전히 제거해라\n"
            "6) 위 규칙들을 다 적용해도 내용 자체가 의미가 통하지 않아서(존재하지 않는 개념을 "
            "묻거나 문맥이 아예 성립하지 않아서) 도저히 자연스러운 질문으로 못 고치겠으면, "
            "억지로 만들어내지 말고 'DISCARD'라는 단어만 출력해라\n"
            "7) 결과는 다듬어진 질문 문장 하나 또는 'DISCARD' 중 하나만 출력해라 - 설명, "
            "따옴표, 다른 말은 절대 붙이지 마라\n\n"
            f"질문 후보: {question}"
        )
