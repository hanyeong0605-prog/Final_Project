...
def chat(message: str, history: list[dict] | None = None, member_id: int | None = None) -> AssistantReply:
    ...
    prompt = (
        ...
        "3. navigate_to는 사용자가 '이동해줘', '열어줘', '보여줘'처럼 이동을 명시적으로 요청한 경우에만 "
        "채워라. 요금·이용권·기능·절차처럼 정보를 묻는 질문에는 절대 페이지 이동만으로 답하지 말고, "
        "근거 자료에 있는 구체적인 내용을 reply에서 먼저 완결되게 답한 뒤 navigate_to는 null로 둬라. "
        "navigate_to를 채웠다면 reply에도 어디로 안내하는지 자연스럽게 언급해라.\n"
