...
def find_page_for_message(message: str) -> SitePage | None:
    """메시지와 관련 있는 "자세히 보러 갈" 페이지를 고른다(Gemini 호출 없음).

    Gemini가 suggested_navigate_to를 비워 보내는 경우를 받아내는 결정적 경로다 - 모델이
    어떻게 답하든 같은 질문에는 항상 같은 페이지가 제안되게 하려는 것이기도 하다.

    여기서 고른 페이지는 답변을 "대체"하는 게 아니라 답변 아래에 붙는 선택지다. 채팅에는
    답(그리고 적합도 순 공고 카드 같은 미리보기)을 그대로 보여주고, 그 아래에서 "자세히
    보려면 이동할까요?"를 예/아니오로 묻는다.
