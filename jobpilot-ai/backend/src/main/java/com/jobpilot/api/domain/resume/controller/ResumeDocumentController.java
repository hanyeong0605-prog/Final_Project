...
public class ResumeDocumentController {
    ...
    private static String introductionFor(JsonNode introductions, int index) { if (!introductions.isArray()) return ""; String[] terms = {"성장", "성격", "학창", "지원"}; for (JsonNode item : introductions) if (item.path("title").asText().replaceAll("\\s+", "").contains(terms[index])) return item.path("content").asText(); return ""; }
