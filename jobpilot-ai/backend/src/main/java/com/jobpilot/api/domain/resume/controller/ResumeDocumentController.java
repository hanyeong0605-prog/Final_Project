...
public class ResumeDocumentController {
    ...
    private static void populateJobKorea(XWPFDocument docx, JsonNode data) {
        ...
        XWPFTable language = tables.get(3); clearRows(language, 1);
        ...
        XWPFTable activities = tables.get(5); clearRows(activities, 1); XWPFTable otherActivities = tables.get(6); clearRows(otherActivities, 1);
    ...
    private static void fillJobKoreaSkills(XWPFTable table, String skills) { java.util.List<String> values = java.util.Arrays.stream(skills.split(",\\s*")).map(String::trim).filter(value -> !value.isBlank()).toList(); int groups = (values.size() + 2) / 3; ensureRows(table, groups + 1); clearRows(table, 1); for (int group = 0; group < groups; group++) { int from = group * 3, to = Math.min(from + 3, values.size()); put(table, group + 1, 0, String.join(" · ", values.subList(from, to))); put(table, group + 1, 1, "활용 가능"); put(table, group + 1, 2, "프로젝트/학습 경험"); } }
    ...
    private static void fillJobKoreaProject(XWPFTable table, JsonNode project, int offset) { if (project == null || project.isMissingNode()) return; put(table, offset + 1, 1, project.path("title").asText()); put(table, offset + 1, 5, projectPeriod(project)); put(table, offset + 2, 1, first(project.path("role").asText(), projectRole(project.path("description").asText()))); put(table, offset + 3, 1, projectOutcome(project)); }
    /** Keep labelled role/technology notes out of the outcome cell; otherwise they stretch the fixed JobKorea table. */
    private static String projectRole(String description) { return labelledProjectValue(description, "역할"); }
    private static String projectOutcome(JsonNode project) { String explicit = first(project.path("result").asText(), project.path("solution").asText(), project.path("problem").asText()); if (!explicit.isBlank()) return trimForCell(explicit); StringBuilder result = new StringBuilder(); for (String line : project.path("description").asText().split("\\r?\\n")) { String trimmed = line.trim(); if (trimmed.isBlank() || trimmed.matches("^(역할|기술|사용기술)\\s*[:：].*")) continue; if (!result.isEmpty()) result.append('\n'); result.append(trimmed); } return trimForCell(result.toString()); }
    private static String labelledProjectValue(String description, String label) { for (String line : description.split("\\r?\\n")) { String trimmed = line.trim(); if (trimmed.matches("^" + java.util.regex.Pattern.quote(label) + "\\s*[:：].*")) return trimmed.replaceFirst("^" + java.util.regex.Pattern.quote(label) + "\\s*[:：]\\s*", "").trim(); } return ""; }
    private static String trimForCell(String value) { if (value == null) return ""; String trimmed = value.trim(); return trimmed.length() <= 420 ? trimmed : trimmed.substring(0, 417).trim() + "..."; }
    ...
    private static void ensureRows(XWPFTable table, int required) { while (table.getNumberOfRows() < required) table.createRow(); }
    ...
    private static void fillProjectDetail(XWPFTable table, JsonNode project, String skills, String targetRole) { if (project == null || project.isMissingNode()) return; put(table, 1, 1, project.path("title").asText()); put(table, 2, 1, projectPeriod(project)); put(table, 3, 1, first(project.path("skills").asText(), skills)); put(table, 4, 1, first(project.path("description").asText(), join(project.path("problem").asText(), project.path("solution").asText()))); put(table, 5, 1, first(project.path("role").asText(), targetRole)); put(table, 6, 1, project.path("result").asText()); put(table, 7, 1, join(project.path("githubUrl").asText(), project.path("deploymentUrl").asText())); }
    ...
    private static String introductionFor(JsonNode introductions, int index) { if (!introductions.isArray()) return ""; String[][] terms = {{"성장"}, {"성격"}, {"학창", "경력", "특기", "습득"}, {"지원", "포부", "입사"}}; for (JsonNode item : introductions) { String title = item.path("title").asText().replaceAll("\\s+", ""); for (String term : terms[index]) if (title.contains(term) && !item.path("content").asText().isBlank()) return item.path("content").asText(); } int usable = 0; for (JsonNode item : introductions) { String content = item.path("content").asText(); if (content.isBlank()) continue; if (usable++ == index) return content; } return ""; }
