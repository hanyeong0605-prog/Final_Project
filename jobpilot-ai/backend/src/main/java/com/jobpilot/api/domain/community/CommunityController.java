...
@RestController @RequestMapping("/api/v1/community") public class CommunityController{
 ...
 @GetMapping("/posts")public CommunityService.PostPage list(@RequestParam String boardType,@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size,@RequestParam(defaultValue="")String query,@RequestParam(defaultValue="RECENT")String sort,Authentication a){return s.list(boardType,me(a),page,size,query,sort);}
 @GetMapping("/my-activities")public CommunityService.ActivityPage myActivities(@RequestParam(defaultValue="0")int page,@RequestParam(defaultValue="20")int size,Authentication a){return s.myActivities(me(a),page,size);}
