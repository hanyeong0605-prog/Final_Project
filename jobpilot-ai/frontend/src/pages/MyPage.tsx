...
export function MyPage() {
  ...
  const [message, setMessage] = useState(""); const [error, setError] = useState(""); const [jobs, setJobs] = useState<JobPosting[]>([]); const [profile, setProfile] = useState<CareerProfile>();
  ...
  return <><PageHeading eyebrow="MY ACCOUNT" title="마이페이지" body="나의 정보, 스펙과 찜한 채용공고를 관리합니다." />
    ...
    {action === "spec" && <section className="panel account-editor spec-editor"><CareerProfileForm initial={profile} onCancel={() => setAction(null)} onSave={async (value) => { const saved = await saveCareerProfile(value); setProfile(saved); if (member) updateMember({ ...member, onboardingCompleted: true }); setMessage("스펙정보를 저장했습니다."); setAction(null); }} /></section>}
