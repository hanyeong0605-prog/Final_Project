...
export function MyPage() {
  ...
  const withdrawalSubmit = (e: FormEvent) => { e.preventDefault(); if (!confirm("회원 정보와 일정이 모두 삭제됩니다. 정말 탈퇴할까요?")) return; void run("withdraw", async () => { await withdraw(withdrawPassword); logout(); navigate("/login", { replace: true }); }, ""); };
  return <><PageHeading eyebrow="MY ACCOUNT" title="마이페이지" body="나의 정보, 스펙과 찜한 채용공고를 관리합니다." />
    ...
    {action === "password" && <section className="panel account-editor"><h2>비밀번호 변경</h2><form onSubmit={passwordSubmit}><label>현재 비밀번호<input required type="password" autoComplete="current-password" value={passwords.current} onChange={(e) => setPasswords({ ...passwords, current: e.target.value })} /></label><label>새 비밀번호<input required type="password" autoComplete="new-password" minLength={8} maxLength={72} value={passwords.next} onChange={(e) => setPasswords({ ...passwords, next: e.target.value })} /></label><button className="primary-button" disabled={submitting === "password"}>{submitting === "password" ? "변경 중..." : "변경하기"}</button></form></section>}
    {action === "withdraw" && <section className="panel account-editor danger-zone"><h2>회원 탈퇴</h2><p>탈퇴하면 회원 프로필, 매칭 결과, 찜과 일정이 모두 삭제됩니다.</p><form onSubmit={withdrawalSubmit}><label>비밀번호 확인<input required type="password" autoComplete="current-password" value={withdrawPassword} onChange={(e) => setWithdrawPassword(e.target.value)} /></label><button className="danger-button" disabled={submitting === "withdraw"}>{submitting === "withdraw" ? "탈퇴 처리 중..." : "회원 탈퇴 진행"}</button></form></section>}
    <SubscriptionSection />
