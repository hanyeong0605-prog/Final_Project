...
export function OpportunityDetailPage() {
  ...
  return <>
    ...
    <section className="panel">
      <span className={`type-badge ${item.status === "EXPIRED" ? "orange" : "blue"}`}>{item.status === "EXPIRED" ? "훈련 종료" : "훈련 진행/예정"}</span>
      <h2>{item.title}</h2>
      <div className="skills">{item.tags.map(tag => <span key={tag}>{tag}</span>)}</div>
      <div className="spec-summary">
        <div><span>훈련기관</span><strong>{item.organization}</strong></div>
        <div><span>훈련 지역</span><strong>{item.address || "정보 없음"}</strong></div>
        <div><span>훈련 대상</span><strong>{item.trainingTarget || "정보 없음"}</strong></div>
        <div><span>문의</span><strong>{item.phone || "정보 없음"}</strong></div>
        ...
        <div><span>만족도</span><strong>{item.satisfactionScore ?? "정보 없음"}</strong></div>
      </div>
