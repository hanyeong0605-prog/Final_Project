...
export function Test2() {
  ...
  useEffect(() => {
    // 주요능력효능감검사 번호인 "10"번으로 문항 데이터를 불러옵니다.[cite: 1]
    fetchTestQuestions("10")
      .then((res) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 💡 주요능력효능감검사에 맞는 쉼표(,) 구분 형식으로 답변 변환[cite: 1]
      const answerString = questions.map((_, idx) => answers[idx]).join(",");

      // 💡 주요능력효능감검사(10번)에 맞는 대상 코드(100209) 페이로드 구성[cite: 1]
      ...
      console.log("주요능력효능감검사 전송 페이로드:", requestPayload);
