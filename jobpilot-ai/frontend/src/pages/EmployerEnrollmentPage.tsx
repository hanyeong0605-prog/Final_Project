...
export function EmployerEnrollmentPage() {
  ...
  return <main className="auth-page"><section className="auth-card">
    ...
    <span className="eyebrow">X1280 ENROLLMENT</span><h1>모바일 기기 등록</h1><p>관리자 승인이 완료된 기업회원만 등록할 수 있습니다.</p>
    {!qr ? <><label>기업회원 아이디<input value={loginId} onChange={(event) => setLoginId(event.target.value)} /></label><label>가입 확인 비밀번호<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><button className="primary-button" disabled={loading || !loginId || !password} onClick={() => void begin()}>{loading ? "QR 요청 중..." : "등록 QR 발급"}</button></> : <div className="passwordless-qr"><QRCodeSVG value={qr} size={220} /><strong>X1280 앱으로 QR을 스캔해 주세요.</strong><span>등록 완료 여부를 자동으로 확인하고 있습니다.</span></div>}
