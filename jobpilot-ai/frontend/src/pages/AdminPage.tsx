...
import { BriefcaseBusiness, CircleCheckBig, Pencil, Search, ShieldCheck, Trash2, UsersRound } from "lucide-react";
...
export function AdminPage() {
  ...
  const hidePosting = async (posting: AdminJobPosting) => {
    ...
    try {
      ...
      setEditing(null);
      setNotice("공고를 숨김 처리했습니다. 데이터와 운영 이력은 보존됩니다.");
      await Promise.all([loadPostings(postingPage.page), getAdminOverview().then(setOverview)]);
    } catch (e) { setError(e instanceof Error ? e.message : "공고 삭제에 실패했습니다."); }
  };
  ...
  return <>
    ...
    <section className="panel admin-panel">
      <div className="admin-panel-heading admin-posting-heading">
        <div><span className="eyebrow">JOB POSTING MANAGEMENT</span><h2>채용공고 관리</h2><p>마감일이 지난 ACTIVE 공고는 10분마다 자동으로 마감 처리되어 일반 화면에서 제외됩니다.</p></div>
        <form onSubmit={(event) => { event.preventDefault(); void loadPostings(0); }}><Search size={16} /><input value={postingQuery} onChange={(event) => setPostingQuery(event.target.value)} placeholder="공고명·회사명 검색" /><button className="outline-button">검색</button></form>
      </div>
      <div className="admin-posting-filters">
        <label>상태<select value={postingStatus} onChange={(event) => { setPostingStatus(event.target.value); }}><option value="ALL">전체 상태</option><option value="ACTIVE">공개</option><option value="CLOSED">마감</option><option value="HIDDEN">숨김</option></select></label>
        <label>정렬<select value={postingSort} onChange={(event) => { setPostingSort(event.target.value); }}><option value="deadline_asc">마감 임박순</option><option value="deadline_desc">마감일 먼 순</option><option value="popular">조회수 높은 순</option><option value="recent">수집 최신순</option></select></label>
      ...
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>공고</th><th>근무지</th><th>마감일</th><th>조회</th><th>상태</th><th>관리</th></tr></thead><tbody>{postingPage.content.map((posting) => {
        const isEditing = editing?.id === posting.id;
        const item = isEditing ? editing : posting;
        return <tr key={posting.id}>
          ...
          <td>{isEditing ? <input className="admin-inline-input" value={item.location ?? ""} onChange={(event) => setEditing({ ...item, location: event.target.value })} placeholder="근무지" /> : posting.location ?? "-"}</td>
          <td>{isEditing ? <input className="admin-inline-input" type="datetime-local" value={toDateTimeInput(item.deadlineAt)} onChange={(event) => setEditing({ ...item, deadlineAt: event.target.value || null })} /> : posting.deadlineAt ? new Date(posting.deadlineAt).toLocaleDateString("ko-KR") : "상시"}</td>
          <td>{posting.viewCount.toLocaleString()}</td>
          <td>{isEditing ? <select value={item.status} onChange={(event) => setEditing({ ...item, status: event.target.value as AdminJobPosting["status"] })}><option value="ACTIVE">공개</option><option value="HIDDEN">숨김</option><option value="CLOSED">마감</option></select> : <span className={`admin-role-badge status-${posting.status.toLowerCase()}`}>{posting.status}</span>}</td>
          <td><div className="admin-table-actions">{isEditing ? <><button className="outline-button" onClick={() => void savePosting()}>저장</button><button className="outline-button" onClick={() => setEditing(null)}>취소</button></> : <><select value={posting.status} onChange={(event) => void updateStatus(posting, event.target.value as AdminJobPosting["status"])}><option value="ACTIVE">공개</option><option value="HIDDEN">숨김</option><option value="CLOSED">마감</option></select><button className="icon-button" title="공고 수정" onClick={() => setEditing(posting)}><Pencil size={15} /></button></>}<button className="icon-button danger" title="공고 숨김" onClick={() => void hidePosting(posting)}><Trash2 size={15} /></button></div></td>
        </tr>;
      })}</tbody></table></div>
