      ...
      <div className="match-evidence-layout" style={evidenceLayoutStyle}>
        ...
        <section className={`growth-plan-pane evidence-pane${paneOpen.plan ? "" : " is-collapsed"}`}>
          ...
          {growthActions.length === 0 ? <div className="growth-plan-empty">현재 확인된 부족 요건이 없습니다.</div> : <div className="growth-plan-list">{growthActions.map((action) => {
            ...
            return <article className="growth-plan-card" key={`${action.category}-${action.title}`}><button type="button" onClick={() => sourceNumbers.length > 0 && focusEvidence(sourceNumbers)}><span>{action.category}</span><strong>{action.title}</strong><p>{action.description}</p>{requirements.length > 0 && <small>공고 근거 {requirements.map((item) => `#${item.sourceNumber}`).join(" · ")} · {action.requirement}</small>}</button>{action.recommendations?.length ? <div className="growth-resource-list">{action.recommendations.map((resource) => <Link key={resource.type} to={`${resource.href}${resource.href.includes("?") ? "&" : "?"}jobPostingId=${job.id}`} className="growth-resource-link"><span>{resource.label}</span><strong>{resource.title}</strong><small>{resource.description}</small><ChevronRight size={13} /></Link>)}</div> : <Link to={`/opportunities?jobPostingId=${job.id}&requirementId=${action.requirementId ?? ""}`} className="growth-detail-link">자세히 보기 <ChevronRight size={14} /></Link>}</article>;
          })}</div>}
          </>}
        </section>
      </div>
