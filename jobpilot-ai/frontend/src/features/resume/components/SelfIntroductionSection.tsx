...
export function SelfIntroductionSection() {
  ...
  const save = async (draft: Draft, index: number) => { if (!draft.title.trim() || !draft.content.trim()) { notify("error", "공백인 항목이 있습니다. 제목과 본문을 입력해 주세요."); return; } const input = { ...draft, primary: false, title: `[${draft.category}] ${draft.title.trim()}` }; setBusy(true); try { const saved = draft.id ? await updateSelfIntroduction(draft.id, input) : await createSelfIntroduction(input); setEntries((current) => [saved, ...current.filter((entry) => entry.id !== saved.id)]); setDrafts((current) => current.filter((_, itemIndex) => itemIndex !== index)); notify("success", "임시 저장되었습니다."); } catch { notify("error", "자기소개서를 저장하지 못했습니다. 잠시 후 다시 시도해 주세요."); } finally { setBusy(false); } };
  const remove = async (id: number) => { if (!confirm("이 자기소개서를 삭제할까요?")) return; setBusy(true); try { await deleteSelfIntroduction(id); setEntries((current) => current.filter((entry) => entry.id !== id)); notify("success", "자기소개서를 삭제했습니다."); } catch { notify("error", "자기소개서를 삭제하지 못했습니다."); } finally { setBusy(false); } };
  return <section className="self-introduction-editor"><div className="jobkorea-entry-title"><div><h3>자기소개서</h3><p>카테고리·제목·본문을 문항별로 여러 개 작성해 관리하세요.</p></div></div>
