...
export function SelfIntroductionSection() {
  ...
  const edit = (entry?: SelfIntroduction) => setDraft(entry ? { id: entry.id, category: categories.find((category) => entry.title.startsWith(`[${category}]`)) ?? "직접 입력", title: entry.title.replace(/^\[[^\]]+\]\s*/, ""), content: entry.content } : emptyDraft());
  const save = async () => { if (!draft?.title.trim() || !draft.content.trim()) return setMessage("제목과 본문을 입력해 주세요."); const input = { ...draft, primary: false, title: `[${draft.category}] ${draft.title.trim()}` }; setBusy(true); try { const saved = draft.id ? await updateSelfIntroduction(draft.id, input) : await createSelfIntroduction(input); setEntries((current) => [saved, ...current.filter((entry) => entry.id !== saved.id)]); setDraft(null); setMessage("임시 저장했습니다."); } catch (error) { setMessage(error instanceof Error ? error.message : "저장에 실패했습니다."); } finally { setBusy(false); } };
