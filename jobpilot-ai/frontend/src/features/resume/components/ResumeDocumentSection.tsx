...
import { CheckCircle2, ChevronRight, Download, FileText, LoaderCircle, Upload } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { applyResumeExtraction, deleteResumeDocument, downloadResumeDocument, extractResumeDocument, generateResumeDocument, getResumeDraftContext, listResumeDocuments, renameResumeDocument, reviewResumeExtraction, type ResumeDocument, type ResumeDraftContext } from "../api/resumeApi";
...
type ExtractionReview = { personalInfo: Record<string, string>; targetRole: string; schoolName: string; major: string; skills: string; project: Record<string, string> };
const objectValue = (value: unknown) => value && typeof value === "object" && !Array.isArray(value) ? value as Record<string, unknown> : {};
const textValue = (value: unknown) => typeof value === "string" ? value : "";
const reviewFrom = (document: ResumeDocument): ExtractionReview => {
  const profile = (document.extractedProfile ?? {}) as Profile; const personal = objectValue(profile.personalInfo);
  const project = objectValue(Array.isArray(profile.portfolios) ? profile.portfolios[0] : undefined);
  return { personalInfo: Object.fromEntries(["name", "birthDate", "email", "phone", "address"].map((key) => [key, textValue(personal[key])])), targetRole: textValue(profile.targetRole), schoolName: textValue(profile.schoolName), major: textValue(profile.major), skills: stringList(profile, "suggestedSkills").join(", "), project: Object.fromEntries(["title", "startedAt", "endedAt", "description"].map((key) => [key, textValue(project[key])])) };
};
...
export function ResumeProfileAnalysisSection() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      setDocuments((current) => [saved, ...current]); setSelected(saved); setReview(reviewFrom(saved)); setReviewOpen(true);
  ...
  const openReview = (document: ResumeDocument) => { setSelected(document); setReview(reviewFrom(document)); setReviewOpen(true); };
  const apply = async (id: number) => {
    setLoading(true); setMessage("");
    try { const reviewed = review ? await reviewResumeExtraction(id, review) : null; if (reviewed) { setSelected(reviewed); setDocuments((current) => current.map((document) => document.id === reviewed.id ? reviewed : document)); } await applyResumeExtraction(id); setReviewOpen(false); setMessage("확인한 정보를 내 스펙에 반영했습니다. 이제 세 가지 양식 중 하나를 골라 이력서 초안을 만들 수 있습니다."); }
  ...
  return <div className="resume-document-section">
    ...
    {selected && <section className="resume-extracted-result"><div className="resume-result-heading"><div><span className="eyebrow">EXTRACTED PROFILE</span><h3>이력서에서 찾은 스펙 정보</h3><p className="resume-review-note">바로 저장하지 않습니다. 누락되거나 애매한 정보만 확인한 뒤 반영하세요.</p></div><button className="primary-button" disabled={loading} onClick={() => openReview(selected)}>확인하고 반영</button></div>{detectedPhoto(extracted) && <div className="resume-detected-photo"><img src={detectedPhoto(extracted) ?? ""} alt="이력서에서 발견한 사진 후보" /><div><strong>이력서에서 사진 후보를 찾았습니다</strong><p>확인 후 반영하면 내 스펙정보의 프로필 사진으로 저장됩니다.</p></div></div>}<div className="resume-profile-suggestions">{[...profileFields, ...detailedProfileFields.map((field) => ({ key: field.label, label: field.label, format: () => field.value(extracted) }))].map((field) => { const raw = extracted[field.key]; const value = field.format ? field.format(raw) : String(raw ?? ""); const found = Boolean(value && value !== "0"); return <article key={field.key} className={found ? "found" : "missing"}><strong>{field.label}</strong>{found ? <p>{value}</p> : <p>이력서에서 발견되지 않았습니다. 확인 단계에서 직접 입력하거나 비워 둘 수 있습니다.</p>}</article>; })}</div>{reviewOpen && review && <section className="resume-extraction-review"><div><span className="eyebrow">CHECK BEFORE SAVE</span><h3>추출 결과를 확인해 주세요</h3><p>새 양식도 같은 공통 스펙으로 정리합니다. 비어 있는 항목만 채우면 개발교육원형·사람인형·잡코리아형 모두에 반영됩니다.</p></div><div className="resume-review-grid">{([['name','이름'], ['birthDate','생년월일'], ['email','이메일'], ['phone','휴대전화'], ['address','주소']] as const).map(([key,label]) => <label key={key}>{label}<input value={review.personalInfo[key] ?? ''} placeholder={`${label}을 확인하거나 입력`} onChange={(event) => setReview((current) => current ? {...current, personalInfo: {...current.personalInfo, [key]: event.target.value}} : current)} /></label>)}<label>희망 직무<input value={review.targetRole} placeholder="예: 백엔드 개발자" onChange={(event) => setReview((current) => current ? {...current, targetRole:event.target.value} : current)} /></label><label>학교명<input value={review.schoolName} placeholder="학교명" onChange={(event) => setReview((current) => current ? {...current, schoolName:event.target.value} : current)} /></label><label>전공<input value={review.major} placeholder="전공" onChange={(event) => setReview((current) => current ? {...current, major:event.target.value} : current)} /></label><label className="wide">보유 기술 (쉼표로 구분)<input value={review.skills} placeholder="Java, Spring Boot, MySQL" onChange={(event) => setReview((current) => current ? {...current, skills:event.target.value} : current)} /></label></div><details className="resume-project-review"><summary>프로젝트 정보도 확인하기 · 선택</summary><div className="resume-review-grid"><label>프로젝트명<input value={review.project.title ?? ''} placeholder="프로젝트명" onChange={(event) => setReview((current) => current ? {...current, project:{...current.project,title:event.target.value}} : current)} /></label><label>시작일<input value={review.project.startedAt ?? ''} placeholder="YYYY-MM 또는 YYYY-MM-DD" onChange={(event) => setReview((current) => current ? {...current, project:{...current.project,startedAt:event.target.value}} : current)} /></label><label>종료일<input value={review.project.endedAt ?? ''} placeholder="YYYY-MM 또는 YYYY-MM-DD" onChange={(event) => setReview((current) => current ? {...current, project:{...current.project,endedAt:event.target.value}} : current)} /></label><label className="wide">프로젝트 설명<textarea value={review.project.description ?? ''} placeholder="맡은 역할, 사용 기술, 결과를 사실 위주로 입력" onChange={(event) => setReview((current) => current ? {...current, project:{...current.project,description:event.target.value}} : current)} /></label></div></details><div className="form-actions"><button className="outline-button" onClick={() => setReviewOpen(false)}>나중에 확인</button><button className="primary-button" disabled={loading} onClick={() => void apply(selected.id)}>{loading ? '반영 중…' : '확인한 내용 반영'}</button></div></section>}{message.includes('세 가지 양식') && <div className="form-actions"><button className="primary-button" onClick={() => navigate('/capability?tool=writer')}>양식 선택하고 초안 만들기 <ChevronRight size={15} /></button></div>}</section>}
    {selected && (autoSelectedSkills.length > 0 || additionalSkillCandidates.length > 0) && <section className="resume-skill-import"><div><span className="eyebrow">SKILL REVIEW</span><h3>기술 스택 반영 계획</h3><p>이력서에서 많이 확인된 기술부터 프로필의 남은 자리만큼 자동 반영합니다.</p></div>{autoSelectedSkills.length > 0 && <div><strong>자동 반영 · {autoSelectedSkills.length}개</strong><div className="resume-skill-chips">{autoSelectedSkills.map((skill) => <span key={skill}>{skill}</span>)}</div></div>}{additionalSkillCandidates.length > 0 && <div><strong>추가 후보 · {additionalSkillCandidates.length}개</strong><p>프로필 40개 제한 때문에 저장하지 않습니다. 기존 기술을 정리한 뒤 필요할 때 추가하세요.</p><div className="resume-skill-chips muted">{additionalSkillCandidates.map((skill) => <span key={skill}>{skill}</span>)}</div><button type="button" className="outline-button" onClick={() => navigate("/capability?tool=profile")}>스킬 관리로 이동</button></div>}</section>}
    {message && <p className="resume-document-message">{message}</p>}
    <section className="resume-document-list"><div className="resume-result-heading"><h3>내 이력서 자료</h3>{selectedIds.length > 0 && <button className="outline-button" disabled={loading} onClick={() => void remove(selectedIds)}>선택 삭제 ({selectedIds.length})</button>}</div>{uploaded.length === 0 ? <p className="empty-state">아직 분석한 이력서 자료가 없습니다.</p> : uploaded.map((document) => <article key={document.id} className="resume-document-item"><input aria-label={`${document.title} 선택`} type="checkbox" checked={selectedIds.includes(document.id)} onChange={() => setSelectedIds((current) => current.includes(document.id) ? current.filter((id) => id !== document.id) : [...current, document.id])} /><div><span>업로드 분석</span><strong>{document.title}</strong><small>{new Intl.DateTimeFormat("ko-KR").format(new Date(document.createdAt))}</small></div><div className="form-actions"><button className="outline-button" onClick={() => setSelected(document)}>분석 결과 보기</button><button className="outline-button" disabled={loading} onClick={() => openReview(document)}>확인 후 반영</button><button className="outline-button" disabled={loading} onClick={() => void remove([document.id])}>삭제</button></div></article>)}</section>
  </div>;
}
...
export function ResumeWritingAssistantSection() {
  ...
  return <div className="resume-document-section">
    <section className="resume-analysis-hero"><div><span className="eyebrow">RESUME ASSISTANT</span><h2>내 역량을 바탕으로 이력서 초안을 작성하세요.</h2><p>저장된 이력·기술·프로젝트의 실제 문장만 불러온 뒤, 선택한 양식에 맞는 수정 가능한 Word 초안을 만듭니다.</p></div></section>
