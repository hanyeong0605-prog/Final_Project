...
const detailedProfileFields: Array<{ label: string; value: (profile: Profile) => string }> = [
  { label: "학력", value: (profile) => [profile.schoolName, profile.major, profile.educationLevel, profile.graduationStatus].filter(Boolean).join(" · ") },
  { label: "경력", value: (profile) => Number(profile.totalCareerMonths) > 0 ? `${profile.totalCareerMonths}개월` : "" },
