...
export type EmployerTalent = { memberId: number; nickname: string; targetRole: string; targetJobFamily: string; preferredLocations: string; experienceType: string; totalCareerMonths: number; technicalSummary?: string | null; portfolioUrl?: string | null; skills: string[]; entries: EmployerTalentEntry[]; certificates: { name: string; issuer?: string | null; acquiredAt?: string | null }[]; selfIntroductions: { title: string; content: string; primary: boolean }[] };
