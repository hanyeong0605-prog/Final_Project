...
export function OnboardingModal() {
  ...
  return <div className="modal-backdrop"><section className="onboarding-modal onboarding-choice" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
    ...
    <div className="onboarding-head"><span className="eyebrow">PERSONALIZED RECOMMENDATION</span><h2 id="onboarding-title">내 스펙으로 더 잘 맞는 공고를 찾아볼까요?</h2><p>희망 직무와 보유 기술을 입력하면 나에게 맞는 채용공고와 지원 준비도를 추천해 드립니다. 스펙은 역량 관리에서 언제든 수정할 수 있어요.</p></div>
    <button className="primary-button onboarding-start" onClick={() => { setDismissed(true); navigate("/capability?tool=profile"); }}>내 스펙 입력하고 맞춤 추천받기<ArrowRight size={17} /></button>
